...
object (self)
  ...
  method bssHandler (f : string) =
    (* TODO:  change this part into OCaml code*)
  ...
  method textProcess (f: string) =
    ...
    (* FIXME: NOP instructions affact the recovery of control flow structure, so I *)
    (* decided to remove them before analyse, however, this could change the *)
    (* binary size, remember in the PLDI15 paper, we emphise the size of processed *)
    (* binaries remind the same. This might be triggered by user input. *)
