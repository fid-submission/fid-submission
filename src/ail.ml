...
object (self)
  ...
  method instrProcess_2 f =
    ...
    *)
    ...
    let i1 = List.nth il 69 in
    let i2 = List.nth il 70 in
    let i3 = List.nth il 71 in

    print_endline @@ pp_print_instr i1;
    print_endline @@ pp_print_instr i2;
    print_endline @@ pp_print_instr i3;

    let ll = [i1; i2; i3] in
    let open Lifter in
    let module SC = Stack_Change in
    let ll' = SC.stack i1 in

    let ll' = ll' @ [i2;i3] in



    let open Symbolic_engine in
    let module SE = Symbolic_engine in
    let il1 = SE.process ll' in
     *)


    let open Bb_symbolic in
    let module BS = BB_symbolic in
    BS.process il' bbl;

    print_string "bb_number:";
    print_endline @@ string_of_int @@ List.length bbl;
