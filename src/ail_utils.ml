...
module Opcode_utils = struct
    ...
    let is_long_opcode op =
      let aux s =
        let ll = String.length s in
        if ll = 4 || ll = 5 then
          begin
            if String.sub s (ll-2) 1  = "l" then
              true
            else
              false
          end
...
module Exp_utils = struct
    ...
    let is_binop e =
      match e with
        | Ptr (BinOP_PLUS (_, _))
        | Ptr (BinOP_PLUS_S (_, _))
        | Ptr (BinOP_MINUS (_, _))
        | Ptr (BinOP_MINUS_S (_, _)) -> true
        | _ -> false
...
module Instr_template = struct
    ...
    (* produce garbage code, can be used for diversification/obfuscation *)
    (* TODO: currently, we only provide a group of three randomly generated
           garbage code; provide more flexible utility should be an easy
           engineering task *)
    let gen_garbage_instrs i iloc =
      let loc = {iloc with loc_label = ""} in
...
module BB_utils = struct
    ...
    (*
       The only difference between bb_map is this method store the basic block object, not its name
     *)

    let bb_map_with_b bbl instrs =
      let bbl = bbl_sort bbl in
...
module Function_visitor = struct
     ...
     (* fb_tbl : hashtbl *)
     let iter_func visitor fb_tbl =
       let aux f tbl =
         visitor f tbl
       in
       Hashtbl.iter aux fb_tbl
...
module Utils = struct
(* other utility functions we need for instrumentation *)
