...
(*
  This could be the outstanding key technique in submitting to top-tier conferences
  But we don't have much time left before the deadline

  Future step, we might remove all references from code sections to data
 sections, as the position of data sections are fixed!
