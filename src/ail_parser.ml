...
object (self)
  ...
  method print_f_to_file fl =
    let oc = open_out_gen [Open_append; Open_creat] 0o666 "call_function_list.txt" in
    List.map (fun f -> f.func_name) fl
    |> List.iter (fun x ->
           output_string oc x;
           output_char oc '\n');
    close_out oc
