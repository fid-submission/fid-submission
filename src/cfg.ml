...
class cfg =
   ...
   *
   * "In general, it's not possible to build CFG precisely from assembly code.
   * Think about indirect calls / branches, for example."
   *
   *  CFG construction is the fundation of diversify transformation as well as
   *  fine-grained CFI, concrete value symbolify
   *
   *  currently, in my design, CFG should process before reassemble plguin.
   *  see cfg_exp's pattern match for details
   *
   *  2014.12.04: How we recover CFG for each intraprocedural
   *
   *
   *   As we alreadly have the bb list for each function, scan the last
   *   instruction for each basic block
   *
   *   For each direct transfer :
   *       1. jmp    -> single successor (get bb info from its name)
   *       2. Jump _ -> double successor
   *       3. call _ -> single successor (get bb info from its name?)
   *
   *   For each indirect transfer :
   *       1. Jump _ *xxx  -> T
   *       2. call *xxx    -> T
   *       3. ret          -> T
   *
   * TODO list:
   *   + update CFG for several rounds (theoretically, until no change at all)
   * *)
  ...
  object(self)
	...
	*)
	...
	in
      ...
      (* TODO : update interprocedural information *)
      let dir_c bnl i =
        let l = get_loc i in
        let bn = bbn_byloc l.loc_addr in
        if bn = "" then assert(false)
