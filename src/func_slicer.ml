...
class func_slicer instrs funcs =
   ...
   *
   *  typical pattern of function prologue in 32 bit x86:
   *
   *      push %ebp
   *      ....

    typical function prologue pattern in 64 bit x86:
       push %rbp
       ....

 * Update 2015-01-31
 *  This instruction pattern can also be considered as a function beginning.
 *
 *  nop
 *  sub $xx, %esp
 *
 *  nop can be substitued with
 *   lea 0x0(%esi,...)
 *   lea 0x0(%edi,...)
 *
 *
 * It is very hard to generate a precise Call Graph based on asm code
 *
 *   thinking of various indirect control flow transfer,
 *   so right now I **only consider direct control flow transfer**.
 *
 *
 *   So firstly I am trying to identify `functions` (begin and end addresses)
 *
 ** Right now, my plan is somehow like this
 ** As for the begin addresses,
 *
 ** I might conservatively consider any assembly code looks like this
...
and also, I might scan the whole problem, identifying all the `call` instruction
with the destination, the consider these function call's destinations
as all the function begin address

The problems are:

1. some of the functions defined in a binary might never be called.

2. some of the `call` has been optimised as `jump` by compiler
(thinking of tail recursive call)


As for the end address, things become more tricky
because multiple `ret` could exist in one single function...

So I am thinking that I might conservatively consider
the range between any nearest **function begin addresses**, as one function..

 *
 * TODO list:
 *   1. Is it good enough?
 *         The test results indicates some too coarse assumption/bug 07/17/2014
 *
 * *)
  ...
  object(self)
    ...
    method filter_addr_by_secs (bl : int list) : int list =
      (*
              FIXME: this s an ad-hoc solution, library object's init section
              information has a section number of [ 9], and it will cause parsing
              errors, so we adjust it in init_sec_adjust.py .
       *)
